...
| `rsync-deploy.yml` | push | Rsyncs `dashboards/shiny-server/` to a host that no longer exists — **safe to delete, along with the directory it copies** |
| `run_daily_scripts.yml` | — | Fully commented out; does nothing |
...
Things that are true right now and want attention.

- **`daily_percent_completion` is pinned to SY24-25.** It calls
  `get_coach_roster(year = "24_25")` and reads the FY25 tracker sheet, so the
  `tl-daily-stats/percent_complete.parquet` it writes daily at 00:30 describes a
  school year that ended over a year ago. It exits 0 every night, so nothing
  flags it. Roll it forward or retire the schedule.
- **`dashboards/shiny-server/` and `rsync-deploy.yml` target a server that no
  longer exists.** Every push still rsyncs that directory to the dead host; all
  dashboards live on Posit Connect. Both are safe to delete.
- **Two EC2 hosts in `~/.ssh/config` (3.17.162.157, 13.59.10.1) have no known
  purpose** — they date from the Shiny Server era. Check the AWS console for
  whether they are still running and billing.
- **Sheet-bound Apps Scripts are infrastructure with no inventory.** At least
  one (the student work grading sheet) calls tl-api on a cadence of its own,
  with the shared secret hardcoded in the script body. The other operational
  sheets have not been audited for bound scripts; the secret belongs in Script
  Properties, and whatever is found belongs in this document.
- **`DATAHUB_PUSH_TOKEN` is a personal gh CLI OAuth token.** It carries write
  access to every repo its owner can reach, and it stops working the next time
  they run `gh auth login` — at which point the Data Hub half of
  `update-architecture.yml` starts failing. A fine-grained PAT scoped to
  `teachinglab.github.io` with Contents: write would fix both problems.
- **Four Connect apps have not been deployed in 6+ months** —
  `ongoing_report_24_25`, `TeachingLLM`, `nps_monitoring_tl`, `tl_scrollytell` —
  and are likely inactive.

## Live Connect inventory

<!-- BEGIN GENERATED: connect-inventory -->

_Auto-generated from the Posit Connect API on 2026-08-11 12:20 EDT by `data_scripts/generate_architecture.R`. Do not edit this section by hand._
